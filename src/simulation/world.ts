...
export class World {
    ...
    // recycle killed organisms and elements to reduce garbage collection
    headOrganismInactive: Organism;
    headElementInactive: Element = null;
     ...
     */
    reset(seed:boolean = true): void {
        for (var i: number = 0; i < this.MAX_WORLD_SIZE; i++) {
            for (var j: number = 0; j < this.MAX_WORLD_SIZE; j++) {
                this.elements[i][j] = null;
            }
        }
        while (this.headOrganism) {
            this.killOrganism(this.headOrganism, true);
        }
        ...
        if (seed) {
            for (var i: number = 0; i < this.parameters.initialSeedCount; i++) {
...
*/            
    ...
    turnCrank(): void {
        ...
        while (organism) {
            ...
            if ((organism.energy < 0 && organism.genome != '*') || organism.lifespan < 0) {
                ...
                // spawn if energy is above threshhold and there aren't more critters than the simulation can handle
                var maximumOrganisms = this.parameters.maximumOrganisms;
                if (organism.genome == '*') {
                    // limit the number of simple '*' critters, as they can overwhelm the simulation and prevent other types
                    // of critters from evolving.
                    maximumOrganisms *= .75;
                }

                if (this.numOrganisms < maximumOrganisms) {
    ...
    serialize():any {
        ...
        for (var x: number = 0; x < this.MAX_WORLD_SIZE; x++) {
            for (var y: number = 0; y < this.MAX_WORLD_SIZE; y++) {
                if (this.elements[x][y] && ! this.elements[x][y].organism) {
                    var type = this.elements[x][y].type.toString();
                    if (! result.elements[type]) {
                        result.elements[type] = [];
                    }
                    result.elements[type].push({x:x,y:y});
                }
            }
        }

        var o:Organism = this.headOrganism;
        while (o) {
            result.organisms.push(o.serialize());
             o = o.next;
        }
    ...
    deserialize(data) {
        ...
        for (var type in data.elements) {
            ...
            var element = (type == ElementType.FOOD.toString()) ? this.foodElement : this.barrierElement;

            data.elements[type].forEach((pos) => {  // foreach statement
                this.put(pos.x, pos.y, element);
            });
        }

        for (var i = 0; i < data.organisms.length; i++) {
            var genome = data.organisms[i].genome;
            var position = new Point(data.organisms[i].positions[0].x, data.organisms[i].positions[0].y);
            var o:Organism = this.createOrganism(data.organisms[i].genome, position);
            o.deserialize(data.organisms[i], this);
        }
    }
}